...
testEmptyLog = do
    log <- mkLog :: IO IntLog
    let val = ServerState (newConfiguration []) (0 :: Int)
    (_,chg) <- commitEntries log (RaftTime 0 0) val
...
testSingleAction = do
    ...
    log1 <- appendEntries log (RaftTime 0 0) [RaftLogEntry 1 $ Cmd $ encode (Add 2)]
    let val = ServerState (newConfiguration []) (1 :: Int)
    entries <- fetchEntries log1 (RaftTime 0 0) 1
    let RaftTime _ lastIndex = lastAppended log1
    assertEqual "Log index should be 0" 0 lastIndex
    assertBool "Log should not be empty" (not $ null entries)
    (log2,ServerState _ chg) <- commitEntries log1 (RaftTime 0 0) val
    assertEqual "Committing simple log did not match expected value" 3 chg
    let RaftTime _ committedIndex = lastCommitted log2
...
testDoubleAction = do
    ...
    log1 <- appendEntries log (RaftTime 0 0) [RaftLogEntry 1 $ Cmd $ encode (Add 2),RaftLogEntry 1 $ Cmd $ encode (Multiply 5)]
    let val = ServerState (newConfiguration []) (1 :: Int)
    entries <- fetchEntries log1 (RaftTime 0 0) 2
    assertBool "Log should not be empty" (not $ null entries)
    assertEqual "Length incorrect" 2 (length entries)
    let (RaftTime term lastIndex) = lastAppended log1
    assertEqual "Appended index incorrect" 1 lastIndex
    (log2,ServerState _ chg) <- commitEntries log1 (RaftTime term 1) val
